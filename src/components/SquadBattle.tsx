...
export function SquadBattle({language,team,onBack,onComplete}:{language:'ru'|'en';team:SquadPlayer[];userChemistry:number;onBack:()=>void;onComplete:()=>void}){
  ...
  const playDuel=()=>{
    ...
    const enemy=available.reduce((best,player)=>playerStats(player)[stat]>playerStats(best)[stat]?player:best);
  ...
  return <section className="squad-battle">
    ...
    {finished?<section className="fatal-finish"><h2>{myScore>aiScore?(en?'You won Squad Battle! 🏆':'Ты выиграл Squad Battle! 🏆'):myScore===aiScore?(en?'Draw! 🤝':'Ничья! 🤝'):(en?'AI wins — improve your squad':'AI победил — собери состав сильнее')}</h2><button onClick={restart}>{en?'New match':'Новый матч'}</button></section>:<button className="battle-play" disabled={!mine} onClick={playDuel}>{!mine?(en?'Choose your card':'Выбери свою карточку'):(en?'AI picks opponent · reveal duel':'AI выберет соперника · открыть дуэль')} <span>→</span></button>}
  </section>;
}
