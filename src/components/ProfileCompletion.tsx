...
export function ProfileCompletion({progress,dailyStreak,language}:{progress:PlayerProgress;dailyStreak:number;language:'ru'|'en'}){
  ...
  return <section className="profile-completion"><div><strong>{remaining===0?(en?'Your profile is complete!':'Профиль заполнен!'):(en?`You're ${remaining} ${remaining===1?'step':'steps'} away from completing your profile.`:`Осталось ${remaining} ${remaining===1?'шаг':'шага'}, чтобы заполнить профиль.`)}</strong><span>{3-remaining}/3</span></div><i><em style={{width:`${((3-remaining)/3)*100}%`}}/></i><ul>{steps.map((step)=><li className={step.done?'done':''} key={step.label}>{step.done?'✓':'○'} {step.label}</li>)}</ul></section>;
}
