...
export function CameraPermission({enabled,language,onChange}:Props){
  ...
  return <section className="camera-permission"><span>📷</span><div><strong>{en?"We'll only use your camera to scan documents when you choose to.":'Мы используем камеру для сканирования документов, только когда ты сам этого захочешь.'}</strong><small>{enabled?(en?'Camera access is enabled.':'Доступ к камере включён.'):(en?'Camera access is optional.':'Доступ к камере необязателен.')}</small>{error&&<em>{error}</em>}<div><button className={enabled?'permission-saved':''} disabled={busy||enabled} onClick={allow}>{enabled?(en?'Saved ✓':'Сохранено ✓'):busy?(en?'Waiting…':'Ожидаем…'):(en?'Allow camera':'Разрешить камеру')}</button><button onClick={()=>onChange(false)}>{enabled?(en?'Turn off':'Отключить'):(en?'Not now':'Не сейчас')}</button></div></div></section>;
}
