...
export function SiteHeader({ page, score, progress, playerProgress, settings, coins, dailyStreak, claimedToday, userEmail, userName, isGuest, onGuest, onSettingsChange, onCoinsChange, onDailyChange, onSignOut, onNavigate }: Props) {
  ...
  return (
    <header className="topbar">
      ...
      <div className="header-stats"><span className="score-pill">✓ {score}</span>{(userEmail||isGuest)&&<span className="header-xp">LVL {Math.floor(playerProgress.xp/500)+1}</span>}{(userEmail||isGuest)&&<span className="header-wallet" title="Field Coins">$ {coins??'—'}</span>}{page === 'training' && <div className="progress-pill"><strong>{progress}</strong></div>}{(userEmail||isGuest)?<div className="profile-wrap"><button className="account-button" onClick={()=>setIsProfileOpen((open)=>!open)} title={nav.profile}>{isGuest?'G':(userName||userEmail||'F').charAt(0).toUpperCase()}</button>{isProfileOpen&&<div className="profile-menu profile-menu--rewards"><span>{nav.account}</span><strong>{isGuest?nav.guest:userName||nav.player}</strong><small>{isGuest?'FieldMind Guest':userEmail}</small><PlayerProgressCard progress={playerProgress} language={settings.language}/><DailyRewards streak={dailyStreak} claimedToday={rewardClaimed} loading={rewardLoading} onClaim={claim}/><SettingsPanel settings={settings} onChange={onSettingsChange}/><button onClick={logout}>{nav.logout}</button></div>}</div>:<div className="guest-entry"><button className="guest-entry__button" onClick={onGuest}>{nav.guestLogin}</button><button className="account-login" onClick={()=>onNavigate('auth')}>{nav.login}</button></div>}</div>
    </header>
  );
}
