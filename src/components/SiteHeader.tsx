...
type Props = { page: Page; score: number; progress: string; playerProgress:PlayerProgress; settings:UserSettings; coins: number | null; dailyStreak:number; claimedToday:boolean; userEmail?: string; userName?: string; isGuest:boolean; onSettingsChange:(settings:UserSettings)=>void; onCoinsChange:(coins:number)=>void; onDailyChange:(streak:number)=>void; onSignOut: () => void; onNavigate: (page: Page) => void };

export function SiteHeader({ page, score, progress, playerProgress, settings, coins, dailyStreak, claimedToday, userEmail, userName, isGuest, onSettingsChange, onCoinsChange, onDailyChange, onSignOut, onNavigate }: Props) {
  ...
  const nav=settings.language==='en'?{home:'Home',training:'Training',games:'Games',quiz:'Quiz',world:'Football world',login:'Sign in',account:'Your account',player:'FieldMind player',guest:'Guest',logout:'Sign out',profile:'Open profile'}:{home:'Главная',training:'Тренировка',games:'Игры',quiz:'Квиз',world:'Футбольный мир',login:'Войти',account:'Твой аккаунт',player:'Игрок FieldMind',guest:'Гость',logout:'Выйти',profile:'Открыть профиль'};
  return (
