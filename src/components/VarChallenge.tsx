...
export function VarChallenge({ language, onBack, onComplete }: Props) {
  ...
  if (finished)
    return (
      <section className="var-finish">
        <div>📺</div>
        <span className="step-label">
          {en ? "REVIEW COMPLETE" : "ПРОВЕРКА ЗАВЕРШЕНА"}
        </span>
        <h1>
          {score >= 6
            ? en
        ...
        <strong>
          {score}
          <small>
            {en ? "of" : "из"} {challenges.length}
          </small>
        </strong>
        <p>
          {score >= 6
            ? en
              ? "You read difficult incidents brilliantly."
              : "Ты отлично читаешь спорные игровые эпизоды."
            : en
              ? "Review the explanations and try again."
              : "Посмотри объяснения и попробуй ещё раз — точность придёт с практикой."}
        </p>
        <button className="play-button" onClick={restart}>
          {en ? "Try again" : "Пройти снова"}
        </button>
        <button className="game-back" onClick={onBack}>
          ← {en ? "All games" : "К списку игр"}
        </button>
      </section>
    );

  return (
    <section className="var-game">
      <button className="game-back" onClick={onBack}>
        ← {en ? "All games" : "Все игры"}
      </button>
      <header className="var-heading">
        <div>
          <div className="eyebrow">
            <span /> {en ? "Referee trainer" : "Судейский тренажёр"}
          </div>
          <h1>{en ? "VAR Challenge" : "VAR-челлендж"}</h1>
        </div>
        <strong>
          {score} / {index + 1}
        </strong>
      ...
      <div className="var-layout">
        <div className={`var-scene var-scene--${challenge.scene}`}>
          <div className="var-screen-label">VAR · REPLAY</div>
          <div className="var-box" />
          <div className="var-goal" />
          <span className="var-attacker">9</span>
          <span className="var-defender">4</span>
          <span className="var-ball">●</span>
          {challenge.scene === "offside" && <i className="var-line" />}
        </div>
        <article className="var-panel">
