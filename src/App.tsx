...
export default function App() {
  ...
  useEffect(()=>{if(!isGuest||user)return;const guest=loadGuestProfile();setCoins(guest.coins);setUnlockedGames(guest.unlockedGames);setOwnedCosmetics(guest.ownedCosmetics);setEquippedCosmetics(guest.equippedCosmetics);setDailyStreak(guest.dailyStreak);setClaimedToday(guest.lastDailyReward===new Date().toISOString().slice(0,10));setPlayerProgress(guest.progress);const savedSettings=localStorage.getItem('fieldmind-guest-settings');if(savedSettings)try{setSettings({...defaultSettings,...JSON.parse(savedSettings) as UserSettings})}catch{localStorage.removeItem('fieldmind-guest-settings')}setGameProfileReady(true)},[isGuest,user]);
