...
import { activeGuestSettingsKey, createDemoProfile, loadGuestProfile, recordGuestActivity, saveGuestProfile } from './lib/guestProfile';
...
export default function App() {
  ...
  useEffect(()=>{if(!isGuest||user)return;const guest=loadGuestProfile();setCoins(guest.coins);setUnlockedGames(guest.unlockedGames);setOwnedCosmetics(guest.ownedCosmetics);setEquippedCosmetics(guest.equippedCosmetics);setDailyStreak(guest.dailyStreak);setClaimedToday(guest.lastDailyReward===new Date().toISOString().slice(0,10));setPlayerProgress(guest.progress);const settingsKey=activeGuestSettingsKey(),savedSettings=localStorage.getItem(settingsKey);if(savedSettings)try{setSettings({...defaultSettings,...JSON.parse(savedSettings) as UserSettings})}catch{localStorage.removeItem(settingsKey)}setGameProfileReady(true)},[isGuest,user]);
