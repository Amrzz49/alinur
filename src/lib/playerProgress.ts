...
export type PlayerProgress={xp:number;skills:Skills;dailyTasks:DailyTasks;dailyRewardClaimed:boolean};
export type Activity='training'|'game'|'match_win';

const defaults:PlayerProgress={xp:0,skills:{vision:10,passing:10,shooting:10,dribbling:10},dailyTasks:{training:0,games:0,wins:0},dailyRewardClaimed:false};
...
export async function loadPlayerProgress():Promise<PlayerProgress|null>{
  ...
  const {data,error}=await supabase.from('game_profiles').select('xp,skills,daily_tasks,daily_task_date,daily_task_reward_date').eq('user_id',auth.user.id).single();
  if(error)throw error;
  const tasks=data.daily_task_date===new Date().toISOString().slice(0,10)?data.daily_tasks:defaults.dailyTasks;
  return {xp:data.xp,skills:data.skills as Skills,dailyTasks:tasks as DailyTasks,dailyRewardClaimed:data.daily_task_reward_date===new Date().toISOString().slice(0,10)};
}
...
export async function recordActivity(activity:Activity,skillChanges:Partial<Skills>={}):Promise<PlayerProgress>{
  ...
  return {xp:result.xp,skills:result.skills as Skills,dailyTasks:result.daily_tasks as DailyTasks,dailyRewardClaimed:profile?.daily_task_reward_date===new Date().toISOString().slice(0,10)};
}
